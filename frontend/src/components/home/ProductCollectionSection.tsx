...
export function ProductCollectionSection({ products }: ProductCollectionSectionProps) {
  return (
    <section className="min-h-screen bg-gradient-to-b from-[#fff8fb] via-[#f8fbff] to-[#f7fbf6] px-5 py-14 sm:px-8 sm:py-16">
      <div className="mx-auto w-full max-w-7xl">
        <div className="flex items-end justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.26em] text-purple-600">Collection</p>
        ...
        {products.length === 0 ? (
          <div className="mt-10 rounded-3xl border border-neutral-200 bg-white/75 px-6 py-10 text-center">
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {products.map((product) => (
              <article
                key={product.id}
                className="group overflow-hidden rounded-[1.8rem] border border-white/75 bg-white/70 shadow-[0_16px_40px_rgba(18,28,45,0.09)] transition-transform duration-300 hover:-translate-y-1"
              >
